// ============================================
// FILE: src/app/(dashboard)/kategori/page.tsx
// ============================================
'use client';
...
export default function KategoriPage() {
    ...
    const isAllSelected = currentKategori.length > 0 &&
        currentKategori.every(k => selectedIds.has(k.id));
    const isSomeSelected = currentKategori.some(k => selectedIds.has(k.id)) && !isAllSelected;

    const handleOpenDialog = (kategoriData?: Kategori) => {
        if (kategoriData) {
            setSelectedKategori(kategoriData);
            setFormData({ title: kategoriData.title });
        } else {
    ...
    const validateForm = async () => {
        ...
        if (!formData.title.trim()) {
            errors.title = 'Judul kategori wajib diisi';
            isValid = false;
        } else if (formData.title.length < 3) {
            errors.title = 'Judul kategori minimal 3 karakter';
            isValid = false;
        } else if (formData.title.length > 100) {
            errors.title = 'Judul kategori maksimal 100 karakter';
            isValid = false;
        } else {
            // Check duplicate
            ...
            if (existing) {
                if (!selectedKategori || existing.id !== selectedKategori.id) {
                    errors.title = 'Judul kategori sudah digunakan';
                    isValid = false;
                }
            }
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const loadingToast = toast.loading(
            selectedKategori ? 'Mengupdate kategori...' : 'Menambahkan kategori...'
        );
    ...
    const handleDelete = async () => {
        ...
        try {
            // Cek apakah kategori sedang digunakan oleh kritik_saran
            ...
            if (kritikSaranData && kritikSaranData.length > 0) {
                toast.error(
                    `Gagal menghapus kategori karena sedang digunakan oleh kritik & saran dari "${kritikSaranData[0].nama}"`,
    ...
    const handleBulkDelete = async () => {
        ...
        setSubmitting(true);
        const loadingToast = toast.loading(`Menghapus ${selectedIds.size} kategori...`);

        try {
            const idsArray = Array.from(selectedIds);

            // Cek apakah ada kategori yang sedang digunakan oleh kritik_saran
            const { data: kritikSaranData, error: checkError } = await supabase
                .from('kritik_saran')
                .select('kategori_id, nama, kategori:kategori_id(title)')
                .in('kategori_id', idsArray);

            if (checkError) throw checkError;

            if (kritikSaranData && kritikSaranData.length > 0) {
                const typedKritikSaranData = kritikSaranData as unknown as KritikSaranCheckResponse[];
                const usedKategori = typedKritikSaranData
                    .map(d => d.kategori?.title || 'Unknown')
                    .join(', ');

                toast.error(
                    `Gagal menghapus karena ${kritikSaranData.length} kategori sedang digunakan oleh kritik & saran: ${usedKategori}`,
            ...
            if (error) {
                if (error.code === '23503') {
                    toast.error('Gagal menghapus beberapa kategori karena sedang digunakan',
                        { id: loadingToast, duration: 5000 });
                } else {
                    throw error;
                }
            ...
            setSelectedIds(new Set());
            toast.success(`${selectedIds.size} kategori berhasil dihapus`, { id: loadingToast });
            await fetchKategori();
            setBulkDeleteDialogOpen(false);
        } catch (error) {
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">Kelola data kategori</p>
                </div>
                                ...
                                {currentKategori.length === 0 ? (
                                    <TableRow>
                                        <TableCell colSpan={5} className="text-center text-muted-foreground h-32">
                                            {searchQuery ? 'Tidak ada data yang sesuai' : 'Belum ada data kategori'}
                                        </TableCell>
                                    </TableRow>
                                ) : (
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="space-y-4 py-4">
                            <div className="space-y-2">
                                <Label htmlFor="title">
                                    Judul Kategori <span className="text-red-500">*</span>
                                </Label>
                                <Input
                                    id="title"
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            ...
                            <strong>{selectedKategori?.title}</strong>?
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
