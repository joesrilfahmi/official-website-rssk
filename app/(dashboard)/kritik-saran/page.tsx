...
import { Plus, Pencil, Trash2, Loader2, Search, RefreshCw, ArrowUpDown, X, Star, Eye, Calendar } from 'lucide-react';
...
export default function KritikSaranPage() {
    ...
    const applyFilters = useCallback(() => {
        ...
        if (kategoriFilter !== 'all') {
            filtered = filtered.filter(item => item.kategori_id === kategoriFilter);
        }

        // Unit filter
        if (unitFilter !== 'all') {
            filtered = filtered.filter(item => item.unit_pelayanan_id === unitFilter);
        }
        ...
        filtered.sort((a, b) => {
            ...
            if (sortField === 'nama') {
                ...
                const aKat = a.kategori?.title || '';
                const bKat = b.kategori?.title || '';
                compareValue = aKat.localeCompare(bKat, 'id');
    ...
    }, [debouncedSearch, statusFilter, kategoriFilter, unitFilter, ratingFilter, dateFrom, dateTo, data, sortField, sortOrder]);
    ...
    }, [fetchKategori, fetchUnitPelayanan, fetchKritikSaran]);
        ...
        return (
            <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((star) => (
                    <Star
                        key={star}
                        className={`h-4 w-4 ${star <= (rating || 0)
    ...
    const showReset = statusFilter !== 'all' ||
        kategoriFilter !== 'all' ||
        unitFilter !== 'all' ||
        ratingFilter !== 'all' ||
        ...
        <div className="space-y-6">
                        ...
                        <div className="flex flex-wrap gap-3">
                            <Popover>
                                <PopoverTrigger asChild>
                                    <Button variant="outline" className="justify-start text-left font-normal">
                                        ...
                                        {dateFrom ? dateFrom.toLocaleDateString('id-ID') : "Dari Tanggal"}
                                    </Button>
                                </PopoverTrigger>
                            ...
                            <Popover>
                                <PopoverTrigger asChild>
                                    <Button variant="outline" className="justify-start text-left font-normal">
                                        <Calendar className="mr-2 h-4 w-4" />
                                        {dateTo ? dateTo.toLocaleDateString('id-ID') : "Sampai Tanggal"}
                                    </Button>
                                </PopoverTrigger>
                                <PopoverContent className="w-auto p-0">
                                    <CalendarComponent
                                ...
                                {currentData.length === 0 ? (
                                    ...
                                    currentData.map((item, index) => (
                                        <TableRow key={item.id} className={!item.is_readed ? 'bg-blue-50 dark:bg-blue-950/20' : ''}>
                                            ...
                                            <TableCell>
                                                <Badge variant="outline">{item.kategori?.title || '-'}</Badge>
                                            </TableCell>
                                            <TableCell>{renderStars(item.rating)}</TableCell>
                                            <TableCell>
                                                <Badge className={item.status === 'read' ? 'bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300 border-green-300' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/40 dark:text-yellow-300 border-yellow-300'}>
                    ...
                    {selectedItem && (
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <Label className="text-muted-foreground">Unit Pelayanan</Label>
                                    <p className="font-medium">{selectedItem.unit_pelayanan?.title || '-'}</p>
                                </div>
                                <div>
                                    <Label className="text-muted-foreground">Kategori</Label>
                                    <p className="font-medium">{selectedItem.kategori?.title || '-'}</p>
                                </div>
                                <div>
                            ...
                            <div>
                                ...
                                <p className="mt-2 p-4 bg-muted rounded-md">{selectedItem.pesan}</p>
                            </div>
                        </div>
                    )}
                </DialogContent>
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>{selectedItem ? 'Edit' : 'Tambah'} Kritik & Saran</DialogTitle>
                    </DialogHeader>
                    <form onSubmit={handleSubmit}>
                        <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <Input
                                            ...
                                            onChange={(e) => {
                                                ...
                                                if (formErrors.no_hp) setFormErrors({ ...formErrors, no_hp: '' });
                                            }}
                                            placeholder="08123456789"
                                            disabled={submitting}
                                            className={formErrors.no_hp ? 'border-red-500' : ''}
                                        />
                                        {formErrors.no_hp && <p className="text-sm text-red-500 mt-1">{formErrors.no_hp}</p>}
                                    </div>
                                </div>
                            )}
                                ...
                                <div>
                                    ...
                                    <Select
                                        ...
                                        onValueChange={(value) => {
                                            ...
                                            if (formErrors.unit_pelayanan_id) setFormErrors({ ...formErrors, unit_pelayanan_id: '' });
                                        }}
                                        disabled={submitting}
                                    >
                                        <SelectTrigger className={formErrors.unit_pelayanan_id ? 'border-red-500' : ''}>
                                    ...
                                    {formErrors.unit_pelayanan_id && <p className="text-sm text-red-500 mt-1">{formErrors.unit_pelayanan_id}</p>}
                                </div>
                                <div>
                                    ...
                                    <Select
                                        ...
                                        onValueChange={(value) => {
                                            ...
                                            if (formErrors.kategori_id) setFormErrors({ ...formErrors, kategori_id: '' });
                                        }}
                                        disabled={submitting}
                                    >
                                        <SelectTrigger className={formErrors.kategori_id ? 'border-red-500' : ''}>
                                    ...
                                    {formErrors.kategori_id && <p className="text-sm text-red-500 mt-1">{formErrors.kategori_id}</p>}
                                </div>
                            </div>
                            <div>
                                <Label>Rating</Label>
                                <div className="flex gap-2 mt-2">
                                    {[1, 2, 3, 4, 5].map((star) => (
                                        ...
                                        >
                                            <Star
                                                className={`h-8 w-8 cursor-pointer transition-colors ${star <= (formData.rating || 0)
                                                    ? 'fill-yellow-400 text-yellow-400'
                            ...
                            <div>
                                ...
                                />
                                {formErrors.pesan && <p className="text-sm text-red-500 mt-1">{formErrors.pesan}</p>}
                            </div>
                        </div>
            ...
            <AlertDialog open={bulkDeleteDialogOpen} onOpenChange={setBulkDeleteDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Apakah Anda yakin ingin menghapus {selectedIds.size} data yang dipilih? Tindakan ini tidak dapat dibatalkan.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel disabled={submitting}>Batal</AlertDialogCancel>
