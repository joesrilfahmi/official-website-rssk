...
export default function ChatWidget() {
  ...
  useEffect(() => {
    ...
    if (messages.length === 0) {
      enqueue(async () => {
        ...
        await sleep(CHAT_DELAY_MS);
        setMessages([
          { id: 1, text: greetingText, sender: "bot", time: nowTime() },
        ]);
        stopTyping();
      });
      void showQuickReplies(mainMenuReplies);
  ...
  const handleDokterByPoli = async (poliId: string) => {
    ...
    try {
      ...
      const { data: poliRow, error: poliErr } = await supabase
        .from("poli")
        .select("id, nama_poli, status")
        .eq("id", poliId)
        .single();
      ...
      stopTyping();
      await botSay(
        `Berikut dokter aktif di ${poliName}. Silakan pilih dokter:`,
      );
      await showQuickReplies([
      ...
      stopTyping();
      await botSay(
        "Maaf, terjadi kesalahan saat mengambil dokter berdasarkan poli.",
      );
      resetToMainMenu();
    }
  };
  ...
  const handleDokterDetail = async (dokterId: string) => {
    ...
    try {
      ...
      const { data: jadwal, error: jadwalErr } = await supabase
        .from("jadwal_dokter")
        .select("id, dokter_id, hari, jam_mulai, jam_selesai, tipe_jadwal")
      ...
      const photoUrl =
        profileVal && isLikelyImageUrl(profileVal) ? profileVal : "";
      ...
      if (sortedReguler.length || sortedEksekutif.length) {
        if (sortedReguler.length) {
          lines.push("Jadwal Praktik BPJS:");
          for (const j of sortedReguler) {
            lines.push(`- ${j.hari}: ${j.jam_mulai} - ${j.jam_selesai}`);
          }
          lines.push("");
        }

        if (sortedEksekutif.length) {
          lines.push("Jadwal Praktik Eksekutif:");
          for (const j of sortedEksekutif) {
            lines.push(`- ${j.hari}: ${j.jam_mulai} - ${j.jam_selesai}`);
          }
          lines.push("");
        }
  ...
  const handlePromoDetail = async (promoId: string) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from("promo")
        .select("id, title, description, picture, status")
        .eq("id", promoId)
        .single();
  ...
  const handleMenuBerita = async () => {
    addMessage("Berita", "user");
    setVisibleQuickReplies([]);
    ...
    try {
      ...
      const { data, error } = await supabase
        .from("berita")
        .select(
          "id, title, description, thumbnail, status, category, created_at",
        )
        .eq("status", "active")
        .order("created_at", { ascending: false })
        .limit(10);
      ...
      const list = (data ?? []) as BeritaRow[];
      if (!list.length) {
        stopTyping();
        await botSay("Saat ini belum ada berita yang tersedia.");
        resetToMainMenu();
        return;
      }
  ...
  const handleBeritaDetail = async (beritaId: string) => {
    setVisibleQuickReplies([]);
    ...
    try {
      ...
      const b = data as BeritaRow;
      addMessage(b.title, "user");
      ...
      if (b.thumbnail) {
        parts.push("");
        parts.push("[Gambar Berita]");
        parts.push(b.thumbnail);
      }

      parts.push("");
      parts.push(`Kategori: ${b.category}`);
      parts.push("");
      ...
      stopTyping();
      await botSay("Maaf, terjadi kesalahan saat mengambil detail berita.");
      resetToMainMenu();
    }
  };
  ...
  const handleQuickReply = (reply: QuickReply) => {
    ...
    switch (reply.action) {
      ...
      case "detail_berita":
        if (reply.value) void handleBeritaDetail(reply.value);
        break;
              ...
              {messages.map((message) => (
                ...
                >
                  ...
                  <div className="flex-1 max-w-[75%]">
                    ...
                    >
                      {message.sender === "bot" &&
                      message.text.includes("[Gambar Promo]") ? (
                        <div className="space-y-3">
                          {(() => {
                            const lines = message.text.split("\n");
                            const idx = lines.findIndex(
                              (l) => l.trim() === "[Gambar Promo]",
                            );
                            const url =
                        ...
                        message.text.includes("[Foto Dokter]") ? (
                        <div className="space-y-3">
                          {(() => {
                            const lines = message.text.split("\n");
                            const idx = lines.findIndex(
                              (l) => l.trim() === "[Foto Dokter]",
                            );
                            const url =
                              idx >= 0 ? (lines[idx + 1] || "").trim() : "";
                            const before =
                        ...
                        message.text.includes("[Gambar Berita]") ? (
                        <div className="space-y-3">
                          {(() => {
                            const lines = message.text.split("\n");
                            const idx = lines.findIndex(
                              (l) => l.trim() === "[Gambar Berita]",
                            );
                            const url =
                              idx >= 0 ? (lines[idx + 1] || "").trim() : "";
                            const before =
